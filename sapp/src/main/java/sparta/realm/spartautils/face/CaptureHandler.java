...
public class CaptureHandler extends View {
	...
	public int mStopped;
	public sdbw.verification_type vt;
...
public capturing_interface cpi=new capturing_interface() {
	...
	sdbw.matching_interface main_matching_interface=new sdbw.matching_interface() {
		@Override
    ...
    public void setup_handler(sdbw.matching_interface main_matching_interface)
	{
	...
	//	FSDK.HImage s_img = new FSDK.HImage();
		...
		if (rotated) {
			...
			if(svars.current_device(mContext)== svars.DEVICE.WALL_MOUNTED.ordinal()) {
		...
		for (int i=0; i<face_count[0]; ++i) {
			...
			if (IDs[i] != -1) {
				...
				if (names[0] != null && names[0].length() > 0) {
					member discovered_member =sd.load_employee(names[0]);
					if(discovered_member ==null) {
						...
						if(vt== sdbw.verification_type.clock_in||vt== sdbw.verification_type.clock_out){
							if(sd.can_clock(vt== sdbw.verification_type.clock_in, discovered_member.sid.value)){
								...
								canvas.drawText(discovered_member.father_name+" "+(vt== sdbw.verification_type.clock_in?"Clocked in":"Clocked out"), (mFacePositions[i].x1+mFacePositions[i].x2)/2, mFacePositions[i].y2+shift, mPaintGREEN);
...
boolean searching=false;
	...
	//	sdbw sd;
		...
		String full_path= svars.WORKING_APP.file_path_employee_data +img_name;
		//FSDK.SaveImageToFile(frame,svars.WORKING_APP.file_path_employee_data +img_name);

		if(FSDK.SaveImageToFile(frame,full_path)== FSDK.FSDKE_OK&&(full_path=svars.WORKING_APP.file_path_employee_data + face_handler.extract_face(svars.WORKING_APP.file_path_employee_data +img_name))!=svars.WORKING_APP.file_path_employee_data +"!!!"){
			//sd=new sdbw(MatchingActivity.acty);
			searching=true;
			sd.load_face_match(full_path, new sdbw.matching_interface() {
				@Override
				public void on_match_complete(boolean match_found, String mils) {
				...
				@Override
				public void on_match_found(String employee_id, String data_index, String match_time,int v_type, int verification_mode) {
					member discovered_member =sd.load_employee(employee_id);
					if(discovered_member ==null) {
